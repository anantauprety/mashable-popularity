...
from mashable_data import getMashableData, getMashableMatrix, SimpleTimer, getPickeledData
...
def runNeuralSimulation(dataTrain, dataTest, holdout, train_M, test_M, hold_M):
    ...
    with SimpleTimer('time to train', outFile):
        net = buildNetwork(trainDS.indim, trainDS.indim/6, trainDS.indim/3, 3, hiddenclass=SigmoidLayer, outclass=SoftmaxLayer)
...
#     dataTrain, dataTest = getMashableData(dataSize)
    ...
    runNeuralSimulation(train, test, holdout, train_M, test_M, hold_M)
